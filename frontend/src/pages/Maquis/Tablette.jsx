...
const Tablette = () => {
  ...
  const confirmerAccomp = () => {
    const lignes = Object.entries(accompSelections).filter(([, q]) => q > 0).map(([id, qty]) => { const p = produits.find(x => x.id === parseInt(id)); return p ? { cle: `${id}__offert_${Date.now()}_${Math.random().toString(36).slice(2)}`, produit_id: p.id, nom: `${p.nom} (Offert)`, prix: 0, quantite: qty, note: '', station_id: null, variante_nom: 'Offert', coefficient: null } : null }).filter(Boolean)
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 32px)', gap: 0 }}>
      ...
      {modalAccomp && (() => {
        ...
        <>
          ...
          <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', backgroundColor: 'white', borderRadius: 16, padding: 24, zIndex: 503, boxShadow: '0 8px 32px rgba(0,0,0,0.25)', width: '90%', maxWidth: 480, maxHeight: '85vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {sucrerieItems.map(s => (
                <div key={s.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderRadius: 12, border: `2px solid ${(accompSelections[s.id] || 0) > 0 ? '#16a34a' : '#e5e7eb'}`, backgroundColor: (accompSelections[s.id] || 0) > 0 ? '#f0fdf4' : 'white' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <button onClick={() => decrementAccomp(s.id)} disabled={!accompSelections[s.id]} style={{ width: 30, height: 30, borderRadius: 6, border: 'none', backgroundColor: accompSelections[s.id] ? '#16a34a' : '#f3f4f6', color: accompSelections[s.id] ? 'white' : '#9ca3af', fontSize: 18, fontWeight: 700, cursor: accompSelections[s.id] ? 'pointer' : 'not-allowed' }}>−</button>
                    <span style={{ fontSize: 15, fontWeight: 700, minWidth: 18, textAlign: 'center' }}>{accompSelections[s.id] || 0}</span>
                    <button onClick={() => incrementAccomp(s)} disabled={reste <= 0} style={{ width: 30, height: 30, borderRadius: 6, border: 'none', backgroundColor: reste > 0 ? '#16a34a' : '#f3f4f6', color: reste > 0 ? 'white' : '#9ca3af', fontSize: 18, fontWeight: 700, cursor: reste > 0 ? 'pointer' : 'not-allowed' }}>+</button>
                  </div>
                </div>
              ))}
              {carafeItems.length > 0 && <p style={{ fontSize: 12, color: '#6b7280', margin: '4px 0 2px', fontWeight: 600 }}>OU — Carafe (utilise tout le quota)</p>}
              {carafeItems.map(c => (
                <button key={c.id} onClick={() => { setPanier(prev => [...prev, { cle: `${c.id}__offert_${Date.now()}`, produit_id: c.id, nom: `${c.nom} (Offert)`, prix: 0, quantite: 1, note: '', station_id: null, variante_nom: 'Offert', coefficient: null }]); setModalAccomp(null); setAccompSelections({}) }}
                  style={{ padding: '11px 14px', border: '2px solid #3b82f6', borderRadius: 12, backgroundColor: '#eff6ff', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                <button onClick={confirmerAccomp} disabled={accompTotal === 0} style={{ flex: 1, padding: '12px 16px', borderRadius: 12, border: 'none', backgroundColor: accompTotal > 0 ? '#16a34a' : '#e5e7eb', color: accompTotal > 0 ? 'white' : '#9ca3af', fontSize: 14, fontWeight: 700, cursor: accompTotal > 0 ? 'pointer' : 'not-allowed' }}>
                  Confirmer ({accompTotal} offert{accompTotal > 1 ? 's' : ''})
                </button>
                <button onClick={() => { setModalAccomp(null); setAccompSelections({}) }} style={{ padding: '12px 16px', borderRadius: 12, border: '2px solid #e5e7eb', backgroundColor: 'white', fontSize: 14, fontWeight: 600, color: '#6b7280', cursor: 'pointer' }}>Sans</button>
              </div>
            </div>
          </div>
        </>
