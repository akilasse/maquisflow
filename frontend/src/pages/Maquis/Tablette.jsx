              ...
              {sucrerieItems.map(s => (
                <div key={s.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderRadius: 12, border: `2px solid ${(accompSelections[s.id] || 0) > 0 ? '#16a34a' : '#e5e7eb'}`, backgroundColor: (accompSelections[s.id] || 0) > 0 ? '#f0fdf4' : 'white' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <button onClick={() => incrementAccomp(s)} disabled={getAccompWeight(s) > reste} style={{ width: 30, height: 30, borderRadius: 6, border: 'none', backgroundColor: getAccompWeight(s) <= reste ? '#16a34a' : '#f3f4f6', color: getAccompWeight(s) <= reste ? 'white' : '#9ca3af', fontSize: 18, fontWeight: 700, cursor: getAccompWeight(s) <= reste ? 'pointer' : 'not-allowed' }}>+</button>
                  </div>
                </div>
              ))}
